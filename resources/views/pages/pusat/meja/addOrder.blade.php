...
@section('content')
    @if (auth()->user()->role == 'staff')
        <x-common.page-breadcrumb pageTitle="Tambah Pesanan" :breadcrumbs="[
            ['label' => 'Daftar Meja', 'url' => route('mejas.viewAll', $meja->pusat_id)],
            ['label' => 'Detail Meja', 'url' => route('mejas.show', $meja->id)],
        ]" />
    ...
    <div x-data="{
        menus: [
            ...
            {
                id: {{ $menu->id }},
                nama_fnb: '{{ $menu->fnb->nama_fnb }}',
                harga: {{ $menu->harga }},
                deskripsi: '{{ $menu->fnb->deskripsi }}',
                foto_fnb: '{{ $menu->fnb->foto_fnb }}',
            }, @endforeach
        ],
                                ...
                                <tr>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="flex items-center justify-center">
                                            <div class="text-sm text-gray-500 dark:text-gray-400" x-text="menu.id">
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="flex items-center justify-center">
                                            <div class="w-10 h-10 overflow-hidden rounded-full">
                                                <img :src="menu.foto_fnb" :alt="menu.nama_fnb">
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-gray-400" x-text="menu.nama_fnb">
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-gray-400" x-text="menu.deskripsi">
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-gray-400" x-text="menu.harga">
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                        <div class="flex justify-center relative gap-3">
                                            <form
                                                :action="`/pusat/mejas/{{ $meja->id }}/storeOrder/{{ $transaction_id }}`"
                                                method="POST">
                                                @csrf
                                                <input type="hidden" name="transaction_header_id"
                                                    :value="'{{ $transaction_id }}'">
                                                <input type="hidden" name="menu_id" :value="menu.id">
                                                <input type="hidden" name="nama_fnb" :value="menu.nama_fnb">
                                                <input type="hidden" name="harga" :value="menu.harga">
                                                <input type="hidden" name="quantity" value="1">

                                                <button type="submit"
                                                    class="rounded-lg bg-blue-600 px-3.5 py-2 text-theme-sm font-medium text-white shadow-theme-xs hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                                                    Tambah
                                                </button>
                                            </form>
                                        </div>
            ...
            <div class="px-6 py-4 border-t border-gray-200 dark:border-white/5">
                <div class="flex items-center justify-between">
                    <button @click="prevPage" :disabled="currentPage === 1"
                        ...
                        class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-3 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/3 dark:hover:text-gray-200 sm:px-3.5">
                            ...
                            <li>
                                <button x-show="page !== '...'" @click="goToPage(page)"
                                    :class="currentPage === page ? 'bg-blue-500 text-white' :
                                        'text-gray-700 hover:bg-blue-500/8 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-500'"
