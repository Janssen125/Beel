@extends('layouts.app')
@section('title', 'Tambah Pusat')
@section('content')
    <x-common.page-breadcrumb pageTitle="Tambah Pusat" :breadcrumbs="[['label' => 'Daftar Pusat', 'url' => route('pusats.index')]]" />
    <div class="flex justify-center w-full">
        <div class="w-full sm:w-8/12">
            <x-common.component-card title="Tambah Pusat">
                <form action="{{ route('pusats.store') }}" method="POST" class="grid gap-6">
                    @csrf
                    <div>
                        ...
                        <select name="pemilik_id"
                            ...
                            @foreach ($pemiliks as $pemilik)
                                <option value="{{ $pemilik->id }}"
                                    {{ old('pemilik_id') == $pemilik->id ? 'selected' : '' }}>
                                    {{ $pemilik->name }}</option>
                            @endforeach
                        </select>
                    </div>
