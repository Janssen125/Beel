@extends('layouts.app')
@section('title', 'Daftar Jenis Meja')
@section('content')
    <x-common.page-breadcrumb pageTitle="Daftar Jenis Meja" />
    <div x-data="{
        jenismejas: [
            @foreach ($jenisMejas as $jm)
        {
            'id': {{ $jm->id }},
            'nama_jenis_meja': '{{ $jm->nama_jenis_meja ?? '-' }}',
        ...
        get filteredJenisMejas() {
            if (!this.search) return this.jenismejas;
    
            const q = this.search.toLowerCase();
    
            return this.jenismejas.filter(k =>
                k.nama_jenis_meja.toLowerCase().includes(q)
            );
        },
    
        get totalPages() {
            return Math.ceil(this.filteredJenisMejas.length / this.itemsPerPage);
        },
    
        get paginatedJenisMejas() {
            const start = (this.currentPage - 1) * this.itemsPerPage;
            return this.filteredJenisMejas.slice(start, start + this.itemsPerPage);
        },
        ...
        openEditModal(jenis_meja) {
            this.form.id = jenis_meja.id;
            this.form.nama_jenis_meja = jenis_meja.nama_jenis_meja;
        ...
        <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Daftar Jenis Meja</h3>
                </div>
                ...
                <div class="max-w-full px-5 overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <template x-for="jenis_meja in paginatedJenisMejas" :key="jenis_meja.id">
                                <tr>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="flex items-center justify-center">
                                            <div class="text-sm text-gray-500 dark:text-gray-400" x-text="jenis_meja.id">
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-gray-400"
                                            x-text="jenis_meja.nama_jenis_meja">
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                        <div class="flex justify-center relative">
                                            <x-ui.button class="edit-button" variant="outline"
                                                @click="openEditModal(jenis_meja)">
                                                Ubah
                                            </x-ui.button>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                        <div class="flex justify-center relative">
                                            <form :action="`/jenis_mejas/${jenis_meja.id}`" method="POST"
                                                @submit.prevent="if(confirm('Apakah Anda yakin ingin menghapus jenis meja ini?')) $el.submit()">
                                                @csrf
                                                @method('DELETE')
                                                <x-ui.button class="delete-button" variant="outline" type="submit">
                                                    Hapus
                                                </x-ui.button>
                                            </form>
                                        </div>
        ...
        <div x-show="showCreateModal" x-transition
            ...
            <div @click.outside="closeModal()" class="w-full max-w-md rounded-xl bg-white p-6 dark:bg-gray-900">
                ...
                <form method="POST" action="{{ route('jenis_mejas.store') }}">
                    @csrf

                    <div class="mb-4">
                        <label class="block mb-1 text-sm">Nama Jenis Meja</label>
                        <input type="text" name="nama_jenis_meja" x-model="form.nama_jenis_meja"
        ...
        <div x-show="showEditModal" x-transition
            ...
            <div @click.outside="closeModal()" class="w-full max-w-md rounded-xl bg-white p-6 dark:bg-gray-900">
                ...
                <form :action="`/jenis_mejas/${form.id}`" method="POST">
                    @csrf
                    @method('PUT')
