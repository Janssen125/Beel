...
<div x-data="{
    ...
    <div class="relative custom-datepicker">
        <input x-ref="dateInput" type="text" id="{{ $id }}" name="{{ $name }}"
            ...
            class="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 bg-transparent text-gray-800
            @error($name)
                border-error-300 focus:border-error-300 focus:ring-error-500/20 dark:border-error-700 dark:focus:border-error-800
            @else
